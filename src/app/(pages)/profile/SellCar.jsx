...
const SellCar = ({user}) => {
    ...
    if (name in formData.location || name in formData.ownerDetails ) {
        setFormData((prevData) => ({
            ...prevData,
            location: {
                ...prevData.location,
                [name]: value, // Update the specific key in 'location'
            },
            ownerDetails: {
                ...prevData.ownerDetails,
                [name]: value, // Update the specific key in 'ownerdetail'
            },
        }));
    } else {
        // Update other fields
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const { data } = await axios.post(`${API_URL}/user/post-car`, payload, getAuthHeaders())
            // console.log(data)
            if (data?.success) {
                setFormData({
                    title: '',
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="mb-4">
                        ...
                        <input
                            type="text"
                            name="contactNumber"
                            value={formData.ownerDetails.contactNumber}
                            onChange={handleChange}
                            placeholder="Enter Car Year"
                            className="w-full border border-purple bg-transparent px-3 py-2.5 rounded"
                        />
